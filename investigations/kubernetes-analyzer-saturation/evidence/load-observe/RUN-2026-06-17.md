...
## Problem summary (full run)

![CXR LOAD-003 — four panels: unstable RPS, rising p95, HPA thrashing, pending pods while node CPU stays low](./grafana-load-003-problem-summary.png)

**One sentence:** Under a **0→200 user** ramp, throughput oscillates, **p95 reaches ~9s**, analyzer HPA scales **1→18** with volatile CPU, and **pending pods** appear — while **node CPU stays ~15%** and memory ~42%.

| Panel | What it shows |
|-------|----------------|
| Load vs throughput | Users ramp to 200; **RPS swings ~10–60** instead of a smooth climb |
| Response time | **p95 ~4s → ~9s**; intermittent **failures/s** spikes at peak load |
| HPA + replicas | Analyzer CPU **spikes to ~400%**; replicas scale up but **thrash** (drops and re-scales) |
| Node + pending | **Node CPU low**; **analyzer pending** blocks from ~16:15 — scheduling pressure, not host exhaustion |

Jaeger (below) explains *why*: **~15–17s cold starts** per new pod + **`context_builder`** (3.8–6.2s) on warm requests.
...
## Grafana — full run (15:36–16:38)

Hero panel: [problem summary](#problem-summary-full-run) above.

**Scrape health = 1** on all series → data trustworthy.

### Signals (complete ramp 0→200 users)

| Signal | Observation |
|--------|-------------|
| Users | Linear ramp **0 → 200** (~15:40–16:20); hold; drop at ~16:28 |
| RPS | **Volatile ~10–60** — throughput does not track users smoothly |
| Latency | p50 **~1s → ~2.5s**; p95 **~4s → ~9s**; failure spikes at peak |
| Analyzer HPA | CPU **spikes to ~400%**; replicas **1 → ~18** with thrashing |
| UI HPA | Low CPU; ~**1** replica throughout |
| Node | CPU **~15%** peak; memory **~18% → ~42%** |
| Pending pods | Analyzer **pending from ~16:15** — HPA wants pods cluster cannot schedule fast enough |

### Mid-ramp snapshot (~80 users, earlier in session)

![Grafana LOAD-003 2×2 during mid-ramp](./grafana-load-003-2x2-live.png)

Same pattern at lower load: rising p95, analyzer scaling, pending pods, low node CPU.
...
| File | Description |
|------|-------------|
| `grafana-load-003-problem-summary.png` | **Canonical** — full 0→200 run, four panels (use for summaries) |
| `grafana-load-003-2x2-live.png` | Mid-ramp snapshot (~80 users) |
